...
class MCTS_Node():
  ...
  def best_child(self):
    return max(self.children.values(), key=lambda child: child.uct_pb())
  ...
  def uct_pb(self):
    ...
    bias = self.bias_factor * self.heuristic_value
...
class MCTSAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState, iterations=100, max_depth=10):
    ...
    children_uct_pb = [child.uct_pb() for _, child in root.children.items()]
    best_action = list(root.children.keys())[np.argmax(children_uct_pb)]
  ...
  def calculate_heuristic(self, state):
      ...
      features = self.getFeatures(state, Directions.STOP)
      weights = self.getWeights(state, Directions.STOP)
      return features * weights
