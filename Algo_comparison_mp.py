...
def run_experiment(params):
    iterations, max_depth, exploration, algo = params
    num_games = 50
    
    print(f"Running experiment with params: {params}")
    command = f"python capture.py -r {algo} -q -n {num_games} --redOpts \"iterations={iterations},depth={max_depth},exploration={exploration}\""
    os.system(command)
    ...
    return [iterations, max_depth, exploration, algo, avg_score, red_win_rate, blue_win_rate, red_elo, blue_elo]
...
def main():
    ...
    iterations_list = [50]
    max_depth_list = [10]
    exploration_factors = [1]
    # rollout_policies = ["random", "heuristic"]
    algos = ["MCTSTeam_heuristic_rollout", "MCTSTeam_UCB1_Tuned", "MCTSTeam_pb"]
    # create HP space
    param_grid = list(itertools.product(iterations_list, max_depth_list, exploration_factors, algos))
    ...
    # Save results
    df = pd.DataFrame(results, columns=["Iterations", "MaxDepth", "ExplorationFactor", "Algorithm", 
                                       "AvgScore", "RedWinRate", "BlueWinRate", "RedElo", "BlueElo"])
    df.to_csv("mcts_algo_results.csv", index=False)
    print("Experiment finished, saved results to mcts_algo_results.csv")
