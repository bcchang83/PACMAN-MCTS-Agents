...
class MCTS_Node():
  ...
  def best_child(self):
    return max(self.children.values(), key=lambda child: child.ucb1_tuned())
  ...
  def ucb1_tuned(self):
    ...
    if len(self.rewards)>=2:
      variance = np.var(self.rewards)
    else:
      variance = 0
      
    exploration = self.exploration_factor * math.sqrt(math.log(self.parent.visits) / self.visits * min(0.25, variance + 2 * math.log(self.parent.visits/self.visits)))
...
class MCTSAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    children_ucb1_tuned = [child.ucb1_tuned() for _, child in root.children.items()]
    best_action = list(root.children.keys())[np.argmax(children_ucb1_tuned)]
    return best_action
